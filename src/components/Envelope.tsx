...
/* The lifafa itself, ported from the Claude Design canvas. Three views on one
   3D card: OPEN (you are filling it), BACK (sealed, the श्री seal) and FRONT
   (what the receiver sees first). The clip-path geometry, the flap curves and
   the note stacking are the canvas's — don't "tidy" the numbers. */
...
export default function Envelope({
  ...
  return (
    <div
      className="relative flex h-[344px] items-end justify-center pb-[34px]"
      style={{ perspective: 1600 }}
    >
      <div className="absolute bottom-[22px] h-5 w-[262px] rounded-[50%] bg-[radial-gradient(ellipse,rgba(0,0,0,.55),transparent_70%)] blur-[3px]" />
      ...
      >
        {/* ── OPEN: you are filling it ─────────────────────────────── */}
        {view === "open" && (
          <div className="absolute inset-0" style={{ transformStyle: "preserve-3d" }}>
            <div
              className="absolute inset-0 rounded-[4px] shadow-[0_20px_36px_rgba(0,0,0,.5),inset_0_1px_0_rgba(255,255,255,.12)]"
              style={paper}
            />
            <div className="absolute inset-x-0 top-0 h-px opacity-65" style={{ background: pal.lace }} />

            {/* the big back flap, standing behind everything */}
            <div
              className="absolute left-0 bottom-[157px] h-[150px] w-[330px] [filter:drop-shadow(0_-6px_14px_rgba(0,0,0,.42))]"
              style={{
            ...
            {/* the message slip, tucked in at an angle */}
            {hasMessage && (
              <div className="lf-slip absolute left-[6px] bottom-[88px] h-[74px] w-[126px] overflow-hidden rounded-[2px] bg-[linear-gradient(158deg,#fbf3e2,#ecdfc6)] px-[9px] py-2 shadow-[0_4px_10px_rgba(0,0,0,.35)]">
            ...
            {/* the notes, flying in and stacking */}
            ...
            {/* the front pocket, with the mithai box and the coin sitting in it */}
            <div
              className="absolute left-0 bottom-0 h-[86px] w-[330px] [filter:drop-shadow(0_-4px_10px_rgba(0,0,0,.3))]"
              style={{
                background: pal.base,
            ...
            >
              {sweet && (
                <div className="lf-tuck absolute left-5 top-5 h-[50px] w-[68px]">
                  <div className="absolute inset-x-0 bottom-0 h-8 rounded-[2px] bg-[linear-gradient(160deg,#f6efdd,#d9c8a9)] shadow-[0_3px_8px_rgba(0,0,0,.45),inset_0_0_0_1px_rgba(0,0,0,.14)]" />
                  <div className="absolute inset-x-0 bottom-[22px] h-[6px] rounded-[2px] bg-[linear-gradient(#fbf6e8,#e6d9bd)] shadow-[0_1px_2px_rgba(0,0,0,.25)]" />
                  <div className="absolute left-[6px] top-[-9px] [filter:drop-shadow(0_2px_3px_rgba(0,0,0,.45))]">
                    <SweetSvg shapes={sweet.sh} width={58} height={50} />
                  </div>
                </div>
              )}
              {s.coin && (
                <div className="lf-coin absolute right-[26px] top-[26px] grid h-[46px] w-[46px] place-items-center rounded-full bg-[conic-gradient(from_210deg,#f3d489,#b8862f,#ffeab8,#9c6c22,#f3d489)] shadow-[0_3px_8px_rgba(0,0,0,.5),inset_0_0_0_2px_rgba(255,255,255,.35)]">
        ...
        {view === "back" && (
          <div className="absolute inset-0" style={{ transform: "rotateY(180deg)" }}>
            <div
              className="absolute inset-0 overflow-hidden rounded-[4px] shadow-[0_22px_40px_rgba(0,0,0,.55),inset_0_0_0_1px_rgba(255,255,255,.1)]"
              style={paper}
            >
              <div
              ...
              />
              <div
                className="absolute left-0 top-0 h-[104px] w-[330px] [filter:drop-shadow(0_6px_10px_rgba(0,0,0,.5))]"
                style={{
        ...
        {/* ── FRONT: what they see first ───────────────────────────── */}
        {view === "front" && (
          <div className="absolute inset-0">
            <div
              className="absolute inset-0 flex flex-col items-center justify-center overflow-hidden rounded-[4px] shadow-[0_22px_40px_rgba(0,0,0,.55),inset_0_0_0_1px_rgba(255,255,255,.1)]"
              style={paper}
            >
              <div
      ...
      {caption && (
        <div className="absolute inset-x-0 bottom-[6px] text-center text-[10.5px] tracking-[.14em] text-[#8d7461]">
          {caption}
        </div>
      )}
    </div>
