  ...
  return (
    ...
    >
      <div className="mx-auto flex w-full max-w-5xl flex-col gap-1.5 px-5 py-2.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:py-3">
        <Link
          href="/"
          className={`text-xl leading-none [font-family:var(--font-wordmark)] ${dark ? "text-[#f2dcae]" : "text-maroon"}`}
        >
          डिजि लिफ़ाफ़ा
        </Link>
      ...
      {/* Portal target. A blog post hangs its contents bar here so it inherits
          the header's real height instead of guessing at it: the nav wraps to
          a second row on narrow phones and the offset is not a constant.
          Empty and zero-height on every other page.
          See app/blog/[slug]/Contents.tsx, which renders nothing without it. */}
      ...
      <div className="mx-auto w-full max-w-5xl px-5 py-12">
        <div className="flex flex-wrap gap-x-10 gap-y-6">
          <div className="min-w-[200px] flex-1">
            <p className="text-lg text-maroon [font-family:var(--font-wordmark)]">
              डिजि लिफ़ाफ़ा
            </p>
          ...
          <nav className="flex flex-col gap-2 text-sm">
            ...
            <Link href="/make" className="text-ink-soft hover:text-ink">
              Make a lifafa
            </Link>
